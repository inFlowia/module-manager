/* Module Manager 0.1 - Динамически подключает модули по необходимости
	Необходимость подключения модуля определяется наличием на странице элемента, удовлетворяющего jQ-селектору.
...
{ // обл. вид
	...
	let modules = [
		...
		{	sel: 'table.h_tips',
			src: '/lib/table-h-tips/table-h-tips.js'
		},
		{	sel: '[onclick *= copy_to_clipboard]',
			src: '/lib/spec.js'
		}
	];
	// Не пытайся подключить здесь rnd.js для rndCh - это событие для него слишком позднее.
	...
	*/
	...
	$(document).ready(function(){
		...
		*/
		...
		update_loaded_scripts();
		modules.forEach((m, i)=>{
			if($('body').find(m.sel + ':first').length){ // потенциальная причина потери производительности. Не уверен, что :first облегчает задачу.
		...
		function watch_always(){ // вынесено в функцию только для удобного отключения
			...
			// Создаём экземпляр наблюдателя с указанной функцией коллбэка
